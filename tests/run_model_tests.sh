...
for test in $1; do
  ...
  ./build/build_atchem2.sh --mechanism=$mechanism_file --shared_lib=$TESTS_DIR/$test/sharedlib --configuration=$TESTS_DIR/$test/configuration --mcm=v3.3.1 &> /dev/null
  exitcode=$?
  ...
  ./atchem2 --output=$TESTS_DIR/$test/output --shared_lib=$TESTS_DIR/$test/sharedlib --configuration=$TESTS_DIR/$test/configuration --constraints=$TESTS_DIR/$test/constraints > $TESTS_DIR/$test/$test.out 2>&1
