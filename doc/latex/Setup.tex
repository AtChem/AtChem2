...
\item \textbf{number of steps} and \textbf{step size}. The duration of
  ...
  \texttt{outputSpecies.config} (see {[}{[}2.6 Config Files{]}{]}),
  and for the environment variables, the photolysis rates, the
  diagnostic variables.\\ For example, a model runtime of 2 hours,
...
\item \textbf{species interpolation method} and \textbf{conditions
  ...
  variables and the photolysis rates, respectively (see {[}{[}2.7
  Constraints{]}{]}).  Two interpolation methods are currently
  implemented in AtChem2: piecewise constant (\texttt{1}) and
  piecewise linear (\texttt{2}). The default option is \emph{piecewise
...
\item \textbf{model start time}. Start time of the model (in seconds)
  ...
  constrained data (see {[}{[}2.7 Constraints{]}{]}).
...
\item \textbf{latitude} and \textbf{longitude}. Geographical
  ...
  MCM photolysis parameterisation (see {[}{[}2.5 Photolysis Rates and
  JFAC{]}{]}).
...
\item \textbf{reaction rates output step size}. Frequency (in seconds)
  ...
  species of interest. For more information go to: {[}{[}2.6 Config
  Files{]}{]}.
\end{itemize}
...
Constraints{]}{]}). Some environment variables can be calculated by
the model (\texttt{CALC}) and some can be deactivated if they are not
used by the model (\texttt{NOTUSED}).
...
set to zero. For more information go to: {[}{[}2.5 Photolysis Rates
and JFAC{]}{]}.
...
extension). See also: {[}{[}2.4 Environment Variables{]}{]}.
...
\item Python scripts to enforce a consistent {[}{[}coding
  style\textbar{}3.2 Style Guide{]}{]}: \texttt{fix\_indent.py},
  \texttt{fix\_style.py}.
