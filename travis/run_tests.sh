...
for test in $1; do
  ...
  ./tools/build.sh $TESTS_DIR/$test/model/configuration/$test.fac $TESTS_DIR/$test/model/configuration $TESTS_DIR/$test/model/configuration mcm
  exitcode=$?
  ...
  ./atchem2 $TESTS_DIR/$test/output $TESTS_DIR/$test/output/reactionRates $TESTS_DIR/$test/model/configuration mcm $TESTS_DIR/$test/model/constraints/species $TESTS_DIR/$test/model/constraints/environment $TESTS_DIR/$test/model/constraints/photolysis > $TESTS_DIR/$test/$test.out
