...
\begin{itemize}
  \item Install
    \texttt{gfortran}, \texttt{cvode}, and \texttt{numdiff}
    \begin{itemize}
    \item linux: use \texttt{apt-get} for \texttt{gfortran},
      ...
      (\texttt{apt-get} could also be used to install
      \texttt{sundials} (including \texttt{cvode}), but it doesn't
      currently hold \texttt{cvode\ 2.9}).
  ...
  \item Build and run a single example of AtChem2. PASS if this exits
    with 0.
  \item Build and run several other examples of AtChem2, using
    different input files. PASS if no differences from the reference
    output files are found, otherwise FAIL. Every test must pass to
    allow the full CI to PASS.
\end{itemize}
