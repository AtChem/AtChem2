...
- update the `Makefile` and move the instructions for the Testsuite to a secondary makefile (`tests/makefile.tests`)
- support chemical mechanisms in KPP format (as generated by the MCM website)
- add use of stoichiometric coefficients in chemical mechanisms
- add option to create user-defined custom Fortran functions
- run AtChem2 in a docker container
...
- create `tex.yml` file to compile and upload the pdf of the manual using GitHub Actions
- change default values in `environmentVariables.config`
