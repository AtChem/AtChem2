...
Although the two versions are usually equivalent, it is recommended to
use the stable version in publications because it can be easily
referenced via the associated
\href{https://www.doi.org/the-identifier/what-is-a-doi}{doi} and
version number. The AtChem2 source code can be obtained in two ways:
...
built only for version 1.2.2. The container is built on Rocky Linux
