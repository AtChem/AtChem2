...
consistent style of coding. Two scripts, \texttt{tools/build/fix\_style.py}
and \texttt{tools/build/fix\_indent.py}, help with keeping the style of the
Fortran code consistent:

\begin{itemize}
\item \texttt{tools/build/fix\_style.py} edits files in-place to try to be
  ...
  therefore, when using the script (by invoking \texttt{python\
    tools/build/fix\_style.py\ filename}), it is strongly recommended to
  have a
...
\item \texttt{tools/build/fix\_indent.py} works similarly, but checks and
