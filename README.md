...
- `model/` contains the the chemical mechanism, and the model configuration, constraints and output. There can be several "models/" directories (with different names).
...
The latest stable version of AtChem2 can be [downloaded here](https://github.com/AtChem/AtChem2/releases). After installing the required dependencies using the scripts in the `tools/install/' directory, copy the file `tools/install/Makefile.skel` to the _Main Directory_ and rename it `Makefile`. Set the variables `CVODELIB`, `OPENLIBMDIR` and `FRUITDIR` in `Makefile` to the paths of CVODE, openlibm and (if installed) FRUIT. To compile AtChem2 using the example chemical mechanism and the default model configuration, execute:

```
./build/build_atchem2.sh ./mcm/mechanism_test.fac
```

The build script converts the chemical mechanism from the FACSIMILE format (`mechanism_test.fac`) to a Fortran-compatible format, and generates the shared library `mechanism.so` in the `model/configuration/` directory. After the build process is completed, an executable file called `atchem2` is created in the _Main Directory_. Set the initial conditions, the required outputs and the other model parameters by editing the files in the `model/configuration/` directory. If required, copy the constraint files to the relevant subdirectory in `model/constraints/`. To run the model, execute:
...
A summary of the main commands, and additional information, is available on the [AtChem2 wiki](https://github.com/AtChem/AtChem2/wiki), together with a list of [Known Issues](https://github.com/AtChem/AtChem2/wiki/Known-Issues) and suggested workarounds.
