...
for file in *.f90 ; do
  ...
  if [ "$file" == "mechanism-rate-coefficients.f90" ] ; then
    continue;
  fi
  echo $file
...
for test in $1; do
  ...
  ./atchem $TESTS_DIR/$test $TESTS_DIR/$test/instantaneousRates $TESTS_DIR/$test/modelConfiguration $TESTS_DIR/$test/speciesConstraints $TESTS_DIR/$test/environmentConstraints | tee $TESTS_DIR/$test.out
