...
for test in $1; do
  ...
  ./build/build_atchem2.sh --mechanism=$TESTS_DIR/$test/model/configuration/$test.fac --shared_lib=$TESTS_DIR/$test/model/sharedlib --configuration=$TESTS_DIR/$test/model/configuration --mcm=v3.3.1 &> /dev/null
  exitcode=$?
  ...
  ./atchem2 --output=$TESTS_DIR/$test/output --shared_lib=$TESTS_DIR/$test/model/sharedlib --configuration=$TESTS_DIR/$test/model/configuration --constraints=$TESTS_DIR/$test/model/constraints > $TESTS_DIR/$test/$test.out 2>&1
